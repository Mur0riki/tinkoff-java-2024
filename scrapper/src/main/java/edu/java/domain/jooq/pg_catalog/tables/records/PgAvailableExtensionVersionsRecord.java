/*
 * This file is generated by jOOQ.
 */
package edu.java.domain.jooq.pg_catalog.tables.records;


import edu.java.domain.jooq.pg_catalog.tables.PgAvailableExtensionVersions;

import java.beans.ConstructorProperties;

import javax.annotation.processing.Generated;

import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;
import org.jooq.Field;
import org.jooq.Record9;
import org.jooq.Row9;
import org.jooq.impl.TableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "https://www.jooq.org",
        "jOOQ version:3.18.7"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes", "this-escape" })
public class PgAvailableExtensionVersionsRecord extends TableRecordImpl<PgAvailableExtensionVersionsRecord> implements Record9<String, String, Boolean, Boolean, Boolean, Boolean, String, String[], String> {

    private static final long serialVersionUID = 1L;

    /**
     * Setter for <code>pg_catalog.pg_available_extension_versions.name</code>.
     */
    public void setName(@Nullable String value) {
        set(0, value);
    }

    /**
     * Getter for <code>pg_catalog.pg_available_extension_versions.name</code>.
     */
    @Nullable
    public String getName() {
        return (String) get(0);
    }

    /**
     * Setter for
     * <code>pg_catalog.pg_available_extension_versions.version</code>.
     */
    public void setVersion(@Nullable String value) {
        set(1, value);
    }

    /**
     * Getter for
     * <code>pg_catalog.pg_available_extension_versions.version</code>.
     */
    @Nullable
    public String getVersion() {
        return (String) get(1);
    }

    /**
     * Setter for
     * <code>pg_catalog.pg_available_extension_versions.installed</code>.
     */
    public void setInstalled(@Nullable Boolean value) {
        set(2, value);
    }

    /**
     * Getter for
     * <code>pg_catalog.pg_available_extension_versions.installed</code>.
     */
    @Nullable
    public Boolean getInstalled() {
        return (Boolean) get(2);
    }

    /**
     * Setter for
     * <code>pg_catalog.pg_available_extension_versions.superuser</code>.
     */
    public void setSuperuser(@Nullable Boolean value) {
        set(3, value);
    }

    /**
     * Getter for
     * <code>pg_catalog.pg_available_extension_versions.superuser</code>.
     */
    @Nullable
    public Boolean getSuperuser() {
        return (Boolean) get(3);
    }

    /**
     * Setter for
     * <code>pg_catalog.pg_available_extension_versions.trusted</code>.
     */
    public void setTrusted(@Nullable Boolean value) {
        set(4, value);
    }

    /**
     * Getter for
     * <code>pg_catalog.pg_available_extension_versions.trusted</code>.
     */
    @Nullable
    public Boolean getTrusted() {
        return (Boolean) get(4);
    }

    /**
     * Setter for
     * <code>pg_catalog.pg_available_extension_versions.relocatable</code>.
     */
    public void setRelocatable(@Nullable Boolean value) {
        set(5, value);
    }

    /**
     * Getter for
     * <code>pg_catalog.pg_available_extension_versions.relocatable</code>.
     */
    @Nullable
    public Boolean getRelocatable() {
        return (Boolean) get(5);
    }

    /**
     * Setter for
     * <code>pg_catalog.pg_available_extension_versions.schema</code>.
     */
    public void setSchema(@Nullable String value) {
        set(6, value);
    }

    /**
     * Getter for
     * <code>pg_catalog.pg_available_extension_versions.schema</code>.
     */
    @Nullable
    public String getSchema() {
        return (String) get(6);
    }

    /**
     * Setter for
     * <code>pg_catalog.pg_available_extension_versions.requires</code>.
     */
    public void setRequires(@Nullable String[] value) {
        set(7, value);
    }

    /**
     * Getter for
     * <code>pg_catalog.pg_available_extension_versions.requires</code>.
     */
    @Nullable
    public String[] getRequires() {
        return (String[]) get(7);
    }

    /**
     * Setter for
     * <code>pg_catalog.pg_available_extension_versions.comment</code>.
     */
    public void setComment(@Nullable String value) {
        set(8, value);
    }

    /**
     * Getter for
     * <code>pg_catalog.pg_available_extension_versions.comment</code>.
     */
    @Nullable
    public String getComment() {
        return (String) get(8);
    }

    // -------------------------------------------------------------------------
    // Record9 type implementation
    // -------------------------------------------------------------------------

    @Override
    @NotNull
    public Row9<String, String, Boolean, Boolean, Boolean, Boolean, String, String[], String> fieldsRow() {
        return (Row9) super.fieldsRow();
    }

    @Override
    @NotNull
    public Row9<String, String, Boolean, Boolean, Boolean, Boolean, String, String[], String> valuesRow() {
        return (Row9) super.valuesRow();
    }

    @Override
    @NotNull
    public Field<String> field1() {
        return PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS.NAME;
    }

    @Override
    @NotNull
    public Field<String> field2() {
        return PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS.VERSION;
    }

    @Override
    @NotNull
    public Field<Boolean> field3() {
        return PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS.INSTALLED;
    }

    @Override
    @NotNull
    public Field<Boolean> field4() {
        return PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS.SUPERUSER;
    }

    @Override
    @NotNull
    public Field<Boolean> field5() {
        return PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS.TRUSTED;
    }

    @Override
    @NotNull
    public Field<Boolean> field6() {
        return PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS.RELOCATABLE;
    }

    @Override
    @NotNull
    public Field<String> field7() {
        return PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS.SCHEMA;
    }

    @Override
    @NotNull
    public Field<String[]> field8() {
        return PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS.REQUIRES;
    }

    @Override
    @NotNull
    public Field<String> field9() {
        return PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS.COMMENT;
    }

    @Override
    @Nullable
    public String component1() {
        return getName();
    }

    @Override
    @Nullable
    public String component2() {
        return getVersion();
    }

    @Override
    @Nullable
    public Boolean component3() {
        return getInstalled();
    }

    @Override
    @Nullable
    public Boolean component4() {
        return getSuperuser();
    }

    @Override
    @Nullable
    public Boolean component5() {
        return getTrusted();
    }

    @Override
    @Nullable
    public Boolean component6() {
        return getRelocatable();
    }

    @Override
    @Nullable
    public String component7() {
        return getSchema();
    }

    @Override
    @Nullable
    public String[] component8() {
        return getRequires();
    }

    @Override
    @Nullable
    public String component9() {
        return getComment();
    }

    @Override
    @Nullable
    public String value1() {
        return getName();
    }

    @Override
    @Nullable
    public String value2() {
        return getVersion();
    }

    @Override
    @Nullable
    public Boolean value3() {
        return getInstalled();
    }

    @Override
    @Nullable
    public Boolean value4() {
        return getSuperuser();
    }

    @Override
    @Nullable
    public Boolean value5() {
        return getTrusted();
    }

    @Override
    @Nullable
    public Boolean value6() {
        return getRelocatable();
    }

    @Override
    @Nullable
    public String value7() {
        return getSchema();
    }

    @Override
    @Nullable
    public String[] value8() {
        return getRequires();
    }

    @Override
    @Nullable
    public String value9() {
        return getComment();
    }

    @Override
    @NotNull
    public PgAvailableExtensionVersionsRecord value1(@Nullable String value) {
        setName(value);
        return this;
    }

    @Override
    @NotNull
    public PgAvailableExtensionVersionsRecord value2(@Nullable String value) {
        setVersion(value);
        return this;
    }

    @Override
    @NotNull
    public PgAvailableExtensionVersionsRecord value3(@Nullable Boolean value) {
        setInstalled(value);
        return this;
    }

    @Override
    @NotNull
    public PgAvailableExtensionVersionsRecord value4(@Nullable Boolean value) {
        setSuperuser(value);
        return this;
    }

    @Override
    @NotNull
    public PgAvailableExtensionVersionsRecord value5(@Nullable Boolean value) {
        setTrusted(value);
        return this;
    }

    @Override
    @NotNull
    public PgAvailableExtensionVersionsRecord value6(@Nullable Boolean value) {
        setRelocatable(value);
        return this;
    }

    @Override
    @NotNull
    public PgAvailableExtensionVersionsRecord value7(@Nullable String value) {
        setSchema(value);
        return this;
    }

    @Override
    @NotNull
    public PgAvailableExtensionVersionsRecord value8(@Nullable String[] value) {
        setRequires(value);
        return this;
    }

    @Override
    @NotNull
    public PgAvailableExtensionVersionsRecord value9(@Nullable String value) {
        setComment(value);
        return this;
    }

    @Override
    @NotNull
    public PgAvailableExtensionVersionsRecord values(@Nullable String value1, @Nullable String value2, @Nullable Boolean value3, @Nullable Boolean value4, @Nullable Boolean value5, @Nullable Boolean value6, @Nullable String value7, @Nullable String[] value8, @Nullable String value9) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        value8(value8);
        value9(value9);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached PgAvailableExtensionVersionsRecord
     */
    public PgAvailableExtensionVersionsRecord() {
        super(PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS);
    }

    /**
     * Create a detached, initialised PgAvailableExtensionVersionsRecord
     */
    @ConstructorProperties({ "name", "version", "installed", "superuser", "trusted", "relocatable", "schema", "requires", "comment" })
    public PgAvailableExtensionVersionsRecord(@Nullable String name, @Nullable String version, @Nullable Boolean installed, @Nullable Boolean superuser, @Nullable Boolean trusted, @Nullable Boolean relocatable, @Nullable String schema, @Nullable String[] requires, @Nullable String comment) {
        super(PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS);

        setName(name);
        setVersion(version);
        setInstalled(installed);
        setSuperuser(superuser);
        setTrusted(trusted);
        setRelocatable(relocatable);
        setSchema(schema);
        setRequires(requires);
        setComment(comment);
        resetChangedOnNotNull();
    }

    /**
     * Create a detached, initialised PgAvailableExtensionVersionsRecord
     */
    public PgAvailableExtensionVersionsRecord(edu.java.domain.jooq.pg_catalog.tables.pojos.PgAvailableExtensionVersions value) {
        super(PgAvailableExtensionVersions.PG_AVAILABLE_EXTENSION_VERSIONS);

        if (value != null) {
            setName(value.getName());
            setVersion(value.getVersion());
            setInstalled(value.getInstalled());
            setSuperuser(value.getSuperuser());
            setTrusted(value.getTrusted());
            setRelocatable(value.getRelocatable());
            setSchema(value.getSchema());
            setRequires(value.getRequires());
            setComment(value.getComment());
            resetChangedOnNotNull();
        }
    }
}
